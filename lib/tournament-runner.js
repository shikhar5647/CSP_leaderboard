...
`;
      ...
      // For Windows, try 'python' or 'py' command
      if (process.platform === 'win32') {
        pythonCommand = 'python'; // Windows usually uses 'python'
      }

      const pythonProcess = spawn(pythonCommand, [tempRunnerPath]);
      ...
      pythonProcess.on('error', (error) => {
        // If Python command fails, try alternative commands
        if (error.code === 'ENOENT') {
          reject(new Error(`Python not found. Please install Python 3.8+ and ensure it's in your PATH. Error: ${error.message}`));
        } else {
